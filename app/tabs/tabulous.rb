Tabulous.setup do

  tabs do

    myproducts_tab do
      text          { 'STEP 1: Add Your Products' }
      link_path     { myproducts_path }
      visible_when  { true }
      enabled_when  { true }
      active_when   { in_action('any').of_controller('myproducts') }
    end

    #profiles_tab do
    #  text          { 'STEP 2 : Skin Profile' }
    #  link_path     { profiles_path }
    #  visible_when  { true }
    #  enabled_when  { current_user.safe_products.present? && current_user.breakout_products.present? }
    #  active_when   { in_action('any').of_controller('profiles') }
    #end

    #analyze_tab do
    #  text          { 'STEP 3 : Analyze' }
    #  link_path     { analyze_index_path }
    #  visible_when  { true }
    #  enabled_when  { current_user.myproducts.present? && current_user.profile.present? }
    #  active_when   { in_action('any').of_controller('analyze') }
    #end

    results_tab do
      text          { 'STEP 2: Add Filters' }
      link_path     { results_path }
      visible_when  { true }
      enabled_when  { current_user.safe_products.present? && current_user.breakout_products.present? }
      active_when   { in_action('any').of_controller('results') }
    end

    recommendation_tab do
      text          { 'STEP 3: Recommendations' }
      link_path     { recommend_path }
      visible_when  { true }
      enabled_when  { current_user.safe_products.present? && current_user.breakout_products.present? }
      active_when   { in_action('any').of_controller('recommend') }
    end

  end

  customize do

    # which class to use to generate HTML
    # :default, :html5, :bootstrap, :bootstrap_pill or :bootstrap_navbar
    # or create your own renderer class and reference it here
    renderer :bootstrap

    # whether to allow the active tab to be clicked
    # defaults to true
    # active_tab_clickable true

    # what to do when there is no active tab for the current controller action
    # :render -- draw the tabset, even though no tab is active
    # :do_not_render -- do not draw the tabset
    # :raise_error -- raise an error
    # defaults to :do_not_render
    # when_action_has_no_tab :do_not_render

    # whether to always add the HTML markup for subtabs, even if empty
    # defaults to false
    # render_subtabs_when_empty false

  end

  # The following will insert some CSS straight into your HTML so that you
  # can quickly prototype an app with halfway-decent looking tabs.
  #
  # This scaffolding should be turned off and replaced by your own custom
  # CSS before using tabulous in production.
  #use_css_scaffolding do
  #  background_color '#ccc'
  #  text_color '#444'
  #  active_tab_color '#fff'
  #  hover_tab_color '#ddd'
  #  inactive_tab_color '#aaa'
  #  inactive_text_color '#888'
  #end

end
